...
export default {
  ...
  methods: {
    getCasesByEmployer() {
      this.fetchData({
        ...
        success: (data) => {
          ...
          this.updateUI(this.casesByEmployerData);
        },
      });
    },
    updateUI(casesByEmployerData) {
      ...
      let data = [];
      for (let [key, value] of Object.entries(casesByEmployerData[0])) {
      ...
      var y = d3.scaleBand().range([height, 0]).domain(data.map(function(d) { return d.employer; })).padding(0.1);
